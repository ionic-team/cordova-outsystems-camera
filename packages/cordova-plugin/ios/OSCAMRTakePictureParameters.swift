...
@available(*, deprecated, message: "This structure will be removed in future versions.")
...
extension IONCAMRTakePhotoOptions {
    ...
    @available(*, deprecated, message: "This initializer will be removed in future versions.")
    convenience init(from parameters: OSCAMRTakePictureParameters) throws {
        ...
        guard let encodingType = IONCAMREncodingType(rawValue: parameters.encodingType) else {
            throw throwError(field: "encodingType")
        }
        guard let cameraDirection = IONCAMRDirection(rawValue: parameters.cameraDirection) else {
        ...
        try self.init(
            ...
            saveToGallery: parameters.saveToPhotoAlbum,
